...
const initialiseCodeMirror = () => {
	...
	const textarea = document.getElementById('code_Pruufs_editor_preview')

	if (textarea) {
		window.code_Pruufs_editor_preview = codeEditor.initialize(textarea)
		return window.code_Pruufs_editor_preview.codemirror
	}
...
export const handleEditorPreviewUpdates = () => {
	...
	const editorSettings = window.code_Pruufs_editor_settings

	for (const setting of editorSettings) {
		const element = document.querySelector(`[name="code_Pruufs_settings[editor][${setting.name}]"]`)
