...
abstract class Admin_Menu {
	 ...
	 * Constructor.
	 *
	 * @param string $name  The Pruuf page short name.
	...
	public function __construct( string $name, string $label, string $title ) {
		...
		$this->base_slug = code_Pruufs()->get_menu_slug();
		$this->slug = code_Pruufs()->get_menu_slug( $name );
	}
	...
	/**
	 * Add a sub-menu to the Pruufs menu.
	 *
	...
	public function load() {
		...
		if ( ! current_user_can( code_Pruufs()->get_cap() ) ) {
			wp_die( esc_html__( 'You are not authorized to access this page.', 'code-Pruufs' ) );
		}

		// Create the Pruuf tables if they are missing.
		$db = code_Pruufs()->db;
	...
	protected function page_title_action_links( array $actions ): array {
		...
		foreach ( $actions as $action ) {
			...
			if ( isset( $_GET['type'] ) && in_array( $_GET['type'], Pruuf::get_types(), true ) ) {
				$url = add_query_arg( 'type', sanitize_key( wp_unslash( $_GET['type'] ) ), $url );
			}

			switch ( $action ) {
				case 'manage':
					$label = _x( 'Manage', 'Pruufs', 'code-Pruufs' );
					break;
				case 'add':
					$label = _x( 'Add New', 'Pruuf', 'code-Pruufs' );
					break;
				case 'import':
					$label = _x( 'Import', 'Pruufs', 'code-Pruufs' );
					break;
				case 'settings':
					$label = _x( 'Settings', 'Pruufs', 'code-Pruufs' );
					break;
				default:
					$label = '';
			}
