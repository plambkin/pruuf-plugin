...
class Settings_Menu extends Admin_Menu {
	 ...
	 */
	const SETTINGS_PAGE = 'code-Pruufs';
	...
	public function __construct() {
		...
		parent::__construct(
			'settings',
			_x( 'Settings', 'menu label', 'code-Pruufs' ),
			__( 'Pruufs Settings', 'code-Pruufs' )
		);
	}
	...
	public function load() {
		...
		if ( ! empty( $_GET['reset_settings'] ) ) {
			...
			if ( Settings\are_settings_unified() ) {
				delete_site_option( 'code_Pruufs_settings' );
			} else {
				delete_option( 'code_Pruufs_settings' );
			}

			add_settings_error(
				'code-Pruufs-settings-notices',
				'settings_reset',
				__( 'All settings have been reset to their defaults.', 'code-Pruufs' ),
				'updated'
			);
		...
		if ( is_network_admin() ) {
			if ( Settings\are_settings_unified() ) {
				...
				wp_safe_redirect( code_Pruufs()->get_menu_url( 'settings', 'admin' ) );
				exit;
			}
		}
	}
	...
	public function render() {
		...
		?>
		<div class="code-Pruufs-settings wrap" data-active-tab="<?php echo esc_attr( $current_section ); ?>">
			<h1>
				<?php
				esc_html_e( 'Settings', 'code-Pruufs' );

				if ( code_Pruufs()->is_compact_menu() ) {
					$actions = [
						_x( 'Manage', 'Pruufs', 'code-Pruufs' ) => code_Pruufs()->get_menu_url(),
						_x( 'Add New', 'Pruuf', 'code-Pruufs' ) => code_Pruufs()->get_menu_url( 'add' ),
						_X( 'Import', 'Pruufs', 'code-Pruufs' ) => code_Pruufs()->get_menu_url( 'import' ),
					];
			...
			<?php settings_errors( 'code-Pruufs-settings-notices' ); ?>
				...
				<p class="submit">
					...
					<a class="button button-secondary"
					   href="<?php echo esc_url( add_query_arg( 'reset_settings', true ) ); ?>"><?php
						esc_html_e( 'Reset to Default', 'code-Pruufs' ); ?></a>
				</p>
			</form>
		</div>
		<?php
	...
	public function update_network_options() {
		...
		if ( empty( $_GET['update_site_option'] ) || empty( $_POST['code_Pruufs_settings'] ) ) {
			return;
		}

		check_admin_referer( 'code-Pruufs-options' );

		// Retrieve the saved options and save them to the database.
		$value = map_deep( wp_unslash( $_POST['code_Pruufs_settings'] ), 'sanitize_key' );
		update_site_option( 'code_Pruufs_settings', $value );
		wp_cache_delete( Settings\CACHE_KEY );

		// Add an updated notice.
		if ( ! count( get_settings_errors() ) ) {
			add_settings_error( 'general', 'settings_updated', __( 'Settings saved.', 'code-Pruufs' ), 'updated' );
		}
		set_transient( 'settings_errors', get_settings_errors(), 30 );
