...
function complete_uninstall_enabled(): bool {
	...
	if ( is_multisite() ) {
		...
		$unified = empty( $menu_perms['Pruufs_settings'] );
	}

	$settings = $unified ? get_site_option( 'code_Pruufs_settings' ) : get_option( 'code_Pruufs_settings' );
...
function uninstall_current_site() {
	...
	$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}Pruufs" ); // cache ok, db call ok.

	delete_option( 'code_Pruufs_version' );
	delete_option( 'recently_activated_Pruufs' );
...
function uninstall_multisite() {
	...
	// Remove network Pruufs table.
	$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}ms_Pruufs" ); // cache ok, db call ok.

	// Remove saved options.
	delete_site_option( 'code_Pruufs_version' );
	delete_site_option( 'recently_activated_Pruufs' );
}

/**
 * Uninstall the Pruufs plugin.
 *
 * @return void
 */
