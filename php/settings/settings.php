...
const CACHE_KEY = 'code_Pruufs_settings';
...
function are_settings_unified(): bool {
	...
	return empty( $menu_perms['Pruufs_settings'] );
}
...
function get_settings_values(): array {
	...
	$saved = get_self_option( are_settings_unified(), 'code_Pruufs_settings', array() );
...
function update_setting( string $section, string $field, $new_value ): bool {
	...
	wp_cache_set( CACHE_KEY, $settings );
	return update_self_option( are_settings_unified(), 'code_Pruufs_settings', $settings );
}
...
function get_settings_sections(): array {
	$sections = array(
		'general' => __( 'General', 'code-Pruufs' ),
		'editor'  => __( 'Code Editor', 'code-Pruufs' ),
	);

	return apply_filters( 'code_Pruufs_settings_sections', $sections );
}
...
function register_plugin_settings() {
	...
	if ( are_settings_unified() ) {
		if ( ! get_site_option( 'code_Pruufs_settings' ) ) {
			add_site_option( 'code_Pruufs_settings', get_default_settings() );
		}
	} elseif ( ! get_option( 'code_Pruufs_settings' ) ) {
		add_option( 'code_Pruufs_settings', get_default_settings() );
	}

	// Register the setting.
	register_setting(
		'code-Pruufs',
		'code_Pruufs_settings',
		array( 'sanitize_callback' => NS . 'sanitize_settings' )
	);

	// Register settings sections.
	foreach ( get_settings_sections() as $section_id => $section_name ) {
		add_settings_section( $section_id, $section_name, '__return_empty_string', 'code-Pruufs' );
	}
	...
	foreach ( get_settings_fields() as $section_id => $fields ) {
		foreach ( $fields as $field_id => $field ) {
			...
			add_settings_field( $field_id, $field['name'], [ $field_object, 'render' ], 'code-Pruufs', $section_id );
		}
	}
