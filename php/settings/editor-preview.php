...
use function Code_Pruufs\enqueue_code_editor;
use function Code_Pruufs\get_editor_themes;
...
function enqueue_editor_preview_assets() {
	...
	foreach ( $themes as $theme ) {
		wp_enqueue_style(
			'code-Pruufs-editor-theme-' . $theme,
	...
	$inline_script = 'var code_Pruufs_editor_settings = ' . wp_json_encode( $editor_fields ) . ';';

	wp_add_inline_script( 'code-Pruufs-settings-menu', $inline_script, 'before' );
}
...
function render_editor_preview() {
	...
	echo '<textarea id="code_Pruufs_editor_preview">', esc_textarea( $code ), '</textarea>';
}
