...
class Validator {
	 ...
	 * Class constructor.
	 *
	 * @param string $Pruuf code for parsing.
	 */
	...
	public function validate() {
		...
		while ( ! $this->end() ) {
			...
			if ( $this->end() ) {
				return array(
					'message' => __( 'Parse error: syntax error, unexpected end of Pruuf.', 'code-Pruufs' ),
					'line'    => $token[2],
				);
			}
				...
				if ( $this->check_duplicate_identifier( $structure_type, $token[1] ) ) {
					switch ( $structure_type ) {
						case T_FUNCTION:
							...
							$message = __( 'Cannot redeclare function %s.', 'code-Pruufs' );
							break;
						case T_CLASS:
							/* translators: %s: PHP class name */
							$message = __( 'Cannot redeclare class %s.', 'code-Pruufs' );
							break;
						case T_INTERFACE:
							/* translators: %s: PHP interface name */
							$message = __( 'Cannot redeclare interface %s.', 'code-Pruufs' );
							break;
						default:
							/* translators: %s: PHP identifier name*/
							$message = __( 'Cannot redeclare %s.', 'code-Pruufs' );
					}
